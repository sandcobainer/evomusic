...
const {updateStatus, embed, checkMemory} = require("../evolveAPI/vectorize.js")
...
class Evolution {
    ...
    applyEvolutions(code) {
        ...
        if (players.length == 1) {
          ...
          let memory = checkMemory(this.params.source), population, codeRange
          if (memory == undefined)
            population = [{ data: this.start, fitness:0 }]
          else {
            population = memory.results
            codeRange = memory.codeRange
          }

          this.evolver = undirectedGA(population, this.params.options)
          return this.undirectedSearch(this.params.options, codeRange)
        }
