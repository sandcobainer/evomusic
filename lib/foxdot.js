...
class FoxDot extends events_1.EventEmitter {
    ...
    {
      let evolutions = obj.evolver.applyEvolutions(code);
      if (!evolutions) return;
      let params = obj.evolver.getParams();
      ...
      let interval =  setInterval(() => {
          ...
          while (cloneEditor.buffer.getLines()[j] != fName){
            j++;
            // if (j>10) {
            //       let clearId = currentEvo[0]['intervalObj']
            //       currentEvo.splice(0,1)
            //       console.log(j, progressRange)
