...
class FoxDot extends events_1.EventEmitter {
    ...
    fitnessUp() {
      ...
      if (parsedResult.pname) {
        ...
        atom.notifications.addWarning('evomusicError: Invalid fitness operations')
      }
    }
    ...
    logEvent(event, msg, code) {
      let time = new Date()
      if(msg) atom.notifications.addInfo(msg)
      if (event) append({ 'event': event, 'code': code, 'tstamp':time.getTime(), 'time': time});
    }
}
