...
class FoxDot extends events_1.EventEmitter {
    ...
    evolutionProcess(code, editor, buffer, obj)
    {
      ...
      '\n' + 'def ' + fName +':'+ '\n'
                    + evolutions[0]
                    + fName + '\n'
                    + `# Evolving from ${env.params.source} to ${env.params.dest}`
      editor.insertText(genCode);

      let funcRange = _.cloneDeep(obj.getRanges()),
          codeRange = _.cloneDeep(funcRange),
          progressRange

      codeRange[0].start.row +=2
      codeRange[0].end.row = codeRange[0].start.row + 1

      console.log(funcRange, codeRange[0].start.row, codeRange[0].end.row)
      ...
      let interval =  setInterval(() => {
          ...
          if(i === evolutions.length - 1) {
            console.log(i, evolutions.length)
