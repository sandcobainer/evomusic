...
function checkMemory(src) {
  if (evoMemory[src] == undefined)
    ...
    return { codeRange : evoMemory[src].codeRange, results : evoMemory[src].results }
}

function updateStatus(msg, src) {
  if (Object.keys(msg)[0]=='codeRange')
    evoMemory[src]['codeRange'] = msg['codeRange']
...
function embed(map, pid) {
  ...
  // ws.send(JSON.stringify({code:map.code, x:embedding.x,y:embedding.y,'players':players}));
}
...
function applyReduction(map) {
  ...
  for (let m in map) {
    vectors.push(vectorize(map[m].data))
  }
