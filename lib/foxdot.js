...
class FoxDot extends events_1.EventEmitter {
    ...
    evaluateRange(range, evolve) {
        ...
        if (evolve) {
          ...
          console.log('Async parallel process starting')
    ...
    evolutionProcess(code, editor, buffer, obj, callback)
    {
      ...
      let codeRange = obj.getRanges()
      let funcRange = _.cloneDeep(codeRange)

      let lines = buffer.getLines()
      // let nextBar = editor.lineTextFromBufferRow(funcRange[0].start.row)
      let autoEvolution = lines[funcRange[0].end.row]
      ...
      let intervalObj = setInterval(() => {
          ...
          while ( buffer.getLines()[j] != autoEvolution )
            j ++;
            
          console.log(funcRange[0])
          funcRange[0].end.row = j;
